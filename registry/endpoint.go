...
func NewEndpoint(index *registrytypes.IndexInfo, metaHeaders http.Header, v APIVersion, proxy string) (*Endpoint, error) {
	tlsConfig, err := newTLSConfig(index.Name, index.Secure)
	if err != nil {
		return nil, err
	}
	endpoint, err := newEndpoint(GetAuthConfigKey(index), proxy, tlsConfig, metaHeaders)
	if err != nil {
		return nil, err
	}
...
func newEndpoint(address, proxy string, tlsConfig *tls.Config, metaHeaders http.Header) (*Endpoint, error) {
	var (
	...
	tr := NewTransport(tlsConfig, proxy)
